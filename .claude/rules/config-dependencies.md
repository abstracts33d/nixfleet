...
- `_shared/host-spec-module.nix` (new flag) → CLAUDE.md flags table, README.md scopes table, docs/src/scopes/
...
- New API endpoint → docs/src/apps/control-plane.md, TECHNICAL.md
- Agent state machine change → TECHNICAL.md lifecycle diagram

## Config files
- `_config/zsh/*` → `core/_home/zsh.nix` + `wrappers/shell.nix`
- `_config/kitty.conf` → `core/_home/simple.nix` + `wrappers/terminal.nix`
- `_config/starship.toml` → `core/_home/starship.nix` + `wrappers/shell.nix`
