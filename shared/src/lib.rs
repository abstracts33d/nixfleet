...
pub struct DesiredGeneration {
    /// Full nix store path, e.g. `/nix/store/abc123...-nixos-system-krach-25.05`
    pub hash: String,
...
mod tests {
    ...
    fn test_desired_generation_serialization_roundtrip() {
        let gen = DesiredGeneration {
            hash: "/nix/store/def456-nixos-system-krach-25.05".to_string(),
