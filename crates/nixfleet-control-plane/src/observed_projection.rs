...
mod tests {
    ...
    fn projection_reflects_each_host_checkin() {
        ...
        checkins.insert("host-02".to_string(), checkin_for("host-02", "def"));
    ...
    fn projection_falls_back_to_failed_on_unknown_host_state() {
        ...
        host_states.insert("host-02".to_string(), "TotallyBogus".to_string());
    ...
    fn projection_round_trips_reverted_state() {
        let mut host_states = HashMap::new();
        host_states.insert("host-02".to_string(), "Reverted".to_string());
        let snap = RolloutDbSnapshot {
            rollout_id: "stable@deadbeef".to_string(),
            channel: "stable".to_string(),
            target_closure_hash: "system-r1".to_string(),
    ...
    fn projection_surfaces_active_rollouts_from_snapshot() {
        let now = Utc::now();
        let mut host_states = HashMap::new();
        host_states.insert("host-02".to_string(), "Healthy".to_string());
        host_states.insert("host-01".to_string(), "ConfirmWindow".to_string());
        let mut last_healthy = HashMap::new();
        last_healthy.insert("host-02".to_string(), now);
        ...
        assert_eq!(
            r.host_states.get("host-02").copied(),
            Some(HostRolloutState::Healthy),
        );
        assert_eq!(
            r.host_states.get("host-01").copied(),
            Some(HostRolloutState::ConfirmWindow),
        );
        assert_eq!(r.last_healthy_since.len(), 1);
        assert_eq!(r.last_healthy_since["host-02"].timestamp(), now.timestamp());
    }
}
