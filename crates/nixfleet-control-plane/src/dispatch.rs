...
pub fn decide_target(
    ...
    // Hold the dispatch only when the agent's most recent fetch failure
    // was for THIS rollout. A failure recorded against a different
    // (older, superseded, no-longer-active) rollout must not pin the
    // agent forever - that's the deadlock that traps a host whose
    // last_fetch_outcome was set during a transient stale-manifest
    // window and never cleared because the agent never sees a new
    // dispatch (chicken-and-egg through the circuit breaker itself).
...
mod tests {
    ...
    fn unmanaged_when_host_not_in_fleet() {
        let fleet = fleet_with("host-02", host(Some("declared-system")));
    ...
    /// Regression cover for the deadlock where an agent recorded
    /// `last_fetch_outcome = VerifyFailed` for one rollout, then the
    /// rollout was superseded by a fresh one - without rollout_id
    /// discrimination the CP held the agent forever, because clearing
    /// `last_fetch_outcome` requires a successful subsequent fetch and
    /// `HoldAfterFailure` prevented any new dispatch from reaching the
    /// agent. With rollout_id discrimination, a failure recorded for a
    /// DIFFERENT rollout must not block dispatch of the current one.
    #[test]
    fn dispatch_when_prior_failure_was_different_rollout() {
        let fleet = fleet_with("test-host", host(Some("declared-system")));
        let mut req = checkin("running-system", Some(FetchResult::VerifyFailed));
        // Mark the prior failure as belonging to a stale, superseded
        // rollout. The current rollout id is content-addressed from the
        // fleet + channel; any other string is "different".
        req.last_fetch_outcome.as_mut().unwrap().rollout_id =
            Some("stale-superseded-rollout-id".to_string());
        ...
        assert!(
            matches!(decision, Decision::Dispatch { .. }),
            "prior failure was for a different rollout; CP must dispatch the fresh one. got: {decision:?}",
        );
    }

    /// Counterpart of `dispatch_when_prior_failure_was_different_rollout`:
    /// when the prior failure's rollout_id MATCHES the current rollout,
    /// the circuit breaker must still hold (preserves the original
    /// "don't hammer an agent stuck fetching the same broken manifest"
    /// intent of `HoldAfterFailure`).
    #[test]
    fn hold_when_prior_failure_was_same_rollout() {
        let fleet = fleet_with("test-host", host(Some("declared-system")));
        // Discover the current rollout id by running a probe checkin
        // with no fetch outcome; CP returns Dispatch with the id.
        let probe = checkin("running-system", None);
        let probed_rollout_id = match decide_target(
            "test-host",
            &probe,
            &fleet,
            TEST_FLEET_HASH,
            ...
            Decision::Dispatch { rollout_id, .. } => rollout_id,
            other => panic!("expected Dispatch with rollout_id from probe, got {other:?}"),
        };
        // Now simulate the agent's prior failure being for the SAME rollout.
        let mut req = checkin("running-system", Some(FetchResult::VerifyFailed));
        req.last_fetch_outcome.as_mut().unwrap().rollout_id = Some(probed_rollout_id);
        assert!(matches!(
            decide_target(
                "test-host",
                &req,
