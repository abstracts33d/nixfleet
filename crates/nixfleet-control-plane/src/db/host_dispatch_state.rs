...
mod tests {
    ...
    fn upsert_replaces_existing_row() {
        ...
        let deadline = Utc::now() + chrono::Duration::seconds(120);
        db.host_dispatch_state()
            .record_dispatch(&dispatch_insert("host-02", "stable@r1", "old", deadline))
            .unwrap();
        db.host_dispatch_state()
    ...
    fn confirm_no_op_when_deadline_passed() {
        ...
        let n = db
            .host_dispatch_state()
            .confirm("host-02", "stable@expired")
            .unwrap();
        assert_eq!(
            n, 0,
    ...
    fn pending_deadlines_picks_past_window() {
        ...
        db.host_dispatch_state()
            .record_dispatch(&dispatch_insert("host-02", "stable@old", "system", past))
            .unwrap();
        let future = Utc::now() + chrono::Duration::seconds(120);
        db.host_dispatch_state()
            .record_dispatch(&dispatch_insert("host-01", "stable@new", "system", future))
            .unwrap();
        let expired = db.host_dispatch_state().pending_deadlines().unwrap();
        assert_eq!(expired.len(), 1);
        assert_eq!(expired[0].0, "host-02");
    ...
    fn record_terminal_flips_matching_rollout() {
        ...
        let deadline = Utc::now() + chrono::Duration::seconds(120);
        db.host_dispatch_state()
            .record_dispatch(&dispatch_insert("host-02", "stable@r1", "system", deadline))
            .unwrap();
        let n = db
            .host_dispatch_state()
    ...
    fn active_rollouts_snapshot_excludes_terminal_states() {
        ...
        let past = Utc::now() - chrono::Duration::seconds(60);
        db.host_dispatch_state()
            .record_dispatch(&dispatch_insert("host-02", "stable@dead", "system", past))
            .unwrap();
        let pairs = vec![("host-02".to_string(), "stable@dead".to_string())];
    ...
    fn active_rollouts_snapshot_pending_surfaces_as_confirmwindow() {
        ...
        assert_eq!(
            r.host_states.get("host-02").map(String::as_str),
            Some("ConfirmWindow"),
        );
    ...
    fn active_rollouts_snapshot_confirmed_uses_host_rollout_state() {
        ...
        db.host_dispatch_state()
            ...
            .unwrap();
        db.host_dispatch_state()
            .confirm("host-02", "stable@abc12345")
            .unwrap();
        mark_healthy(&db, "host-02", "stable@abc12345", now);
        let snap = db.host_dispatch_state().active_rollouts_snapshot().unwrap();
        assert_eq!(snap.len(), 1);
        let r = &snap[0];
        assert_eq!(
    ...
    fn active_rollouts_snapshot_uses_explicit_channel_for_sha_rollout_id() {
        ...
        let mut row = dispatch_insert("host-02", sha_rollout, "system-r1", future);
    ...
    fn pending_deadlines_skips_deferred_rows() {
        ...
        db.host_dispatch_state()
            ...
            .unwrap();
        db.host_dispatch_state()
            .mark_deferred("host-02", "stable@deferred")
            .unwrap();
        ...
        assert_eq!(
            expired[0].0, "host-01",
            "deferred host-02 must not appear in expired set"
        );
    }
    ...
    fn orphan_confirm_lands_both_rows_atomically() {
        ...
        assert_eq!(
            r.host_states.get("host-02").map(|s| s.as_str()),
