...
/// Example suffix for canonical agent CNs (`agent-<machineId>.<suffix>`).
...
mod cn_helpers_tests {
    ...
    fn canonicalises_bare_machine_id() {
        assert_eq!(
            canonical_agent_cn("host-01", "fleet.example.com"),
            "agent-host-01.fleet.example.com"
        );
    }
    ...
    fn extracts_machine_id_from_canonical_cn() {
        assert_eq!(
            extract_machine_id("agent-host-01.fleet.example.com", "fleet.example.com"),
            "host-01"
        );
    }
    ...
    fn extract_passes_through_when_suffix_does_not_match() {
        ...
        assert_eq!(
            extract_machine_id("agent-host-01.other.example", "fleet.example.com"),
            "agent-host-01.other.example"
        );
    }

    #[test]
    fn canonicalisation_roundtrips_through_extract() {
        for id in ["host-01", "host-02", "host-04", "machine-with-dashes"] {
            let canonical = canonical_agent_cn(id, "fleet.example.com");
            assert_eq!(
                extract_machine_id(&canonical, "fleet.example.com"),
                id,
...
mod ca_signer_tests {
    ...
    fn file_ca_signer_round_trips() {
        ...
        let mut agent_params = CertificateParams::new(vec!["host-01".to_string()]).unwrap();
        agent_params
            .distinguished_name
            .push(rcgen::DnType::CommonName, "host-01");
        ...
        assert_eq!(cn_str, "agent-host-01.fleet.example.com");
    }
}
...
";
    ...
    /// single-block SEC1 PEM that rcgen accepts. This is the shape the operator's
    ...
    /// `-text` mode emits - same as the operator's pre-fix `fleet-ca-key.age`.
    #[test]
