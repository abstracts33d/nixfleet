...
fn verify_attestation_signature(
    ...
    if let Err(err) = verifying_key.verify(&canonical, &signature) {
        // Distinguish two failure modes against the SAME verify error:
        //
        ...
        //
        // 2. TAMPER: agent's rollout_id MATCHES CP's projection but the
        ...
        // attestation, fall back to unattested clamp). Only the log
        // level + framing differs.
        let agent_signed_rollout_id = req
            .last_evaluated_target
            .as_ref()
            .map(|t| t.rollout_id.as_str());
        if agent_signed_rollout_id == Some(rollout_id) {
            tracing::warn!(
                host = %req.hostname,
                rollout = %rollout_id,
                error = %err,
            ...
            tracing::debug!(
                host = %req.hostname,
                agent_rollout_id = ?agent_signed_rollout_id,
                cp_rollout_id = %rollout_id,
                error = %err,
                "soak recovery: stale rollout_id (snapshot re-sign without dispatch); falling back to unattested clamp",
            );
        }
        return false;
    }
