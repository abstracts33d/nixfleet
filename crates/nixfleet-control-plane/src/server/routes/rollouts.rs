...
/// bytes as `application/json`. mTLS auth is enforced by the
/// router-level `require_cn_layer`; the handler itself is just a
/// disk lookup + content-type stamp.
...
/// bytes as `application/octet-stream`. Pairs with the manifest
/// endpoint; same router-level mTLS auth.
