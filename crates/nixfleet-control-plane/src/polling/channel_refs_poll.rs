...
mod tests {
    ...
    fn fleet_edge_then_stable() -> FleetResolved {
        let mut f = FleetBuilder::new()
            ...
            .host_closure("host-05", "host-05-closure-new")
            .host("host-01", "stable")
            .host_tag("host-01", "dev")
            .host_closure("host-01", "host-01-closure-new")
            .policy_waves("p", vec![])
            .build();
    ...
    /// rev's edge rollout (host-05=Converged, terminal-for-ordering) would
    ...
    async fn stale_predecessor_rollout_does_not_unblock_new_successor() {
        ...
        seed_old_rollout(&db, "old-edge-rid", "edge", "host-05");
        seed_old_rollout(&db, "old-stable-rid", "stable", "host-01");
    ...
    async fn successor_opens_once_current_predecessor_converges() {
        ...
        // Simulate host-05 activating + converging on the new edge rollout.
        db.host_dispatch_state()
            .record_dispatch(&DispatchInsert {
                hostname: "host-05",
                ...
                channel: "edge",
                wave: 0,
                target_closure_hash: "host-05-closure-new",
