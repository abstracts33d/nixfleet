...
async fn converged_at_dispatch_does_not_leak_dispatch_history_rows() {
    ...
    // Count ALL history rows (any terminal_state): converged-at-dispatch
    // rows are born-terminal (`terminal_state='converged-at-dispatch'`)
    // since the migration that closed fleet-status A7, so a
    // `terminal_state IS NULL` filter would always read 0 here. The
    // semantic property under test is row-count uniqueness, not row
    // open-ness.
    let count_history_rows = || -> i64 {
        let conn = rusqlite::Connection::open(&db_path).unwrap();
        conn.query_row(
            "SELECT COUNT(*) FROM dispatch_history WHERE hostname = ?1",
            rusqlite::params!["test-host"],
            |r| r.get(0),
        )
        .unwrap()
    };
    let after_first = count_history_rows();
    assert_eq!(
        after_first, 1,
        "first converged-at-dispatch materialises exactly one dispatch_history row",
    );

    // Verify the row was born terminal (no soak / no operator confirm).
    {
        let conn = rusqlite::Connection::open(&db_path).unwrap();
        let terminal: Option<String> = conn
            .query_row(
                "SELECT terminal_state FROM dispatch_history WHERE hostname = ?1",
                rusqlite::params!["test-host"],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(
            ...
            "born-terminal: trace must show converged-at-dispatch immediately, not `open`",
        );
    }
    ...
    let after_repeats = count_history_rows();
    assert_eq!(
...
async fn converged_at_dispatch_is_idempotent_under_concurrent_checkins() {
    ...
    // Assertion 1: exactly one dispatch_history row (any terminal_state).
    ...
    assert_eq!(
        history_rows, 1,
        "concurrent converged-at-dispatch must produce EXACTLY one dispatch_history row \
