...
/// Stage 1: edge rollout opened, host-05 dispatched + Activating
/// (predecessor in-flight, NOT terminal-for-ordering).
///
/// Stable rollout opens but is held by channelEdges. host-01's
...
fn stage1_edge_active_stable_held_in_both_modes() {
    ...
    db.host_dispatch_state()
        .record_dispatch(&DispatchInsert {
            ...
            channel: "edge",
            wave: 0,
            target_closure_hash: "host-05-closure",
    ...
    // Dispatch mode: host-01 blocked.
    let v_dispatch = gate_for(&observed, &fleet, "host-01", GateMode::Dispatch);
    // Reconcile mode: ALSO blocked - predecessor is in observed and active-for-ordering.
    let v_reconcile = gate_for(&observed, &fleet, "host-01", GateMode::Reconcile);
...
/// Stage 2: host-05 Soaked (terminal-for-ordering). Stable's
...
/// dispatched. The dispatch happened (host-01 Healthy in observed),
...
/// channelEdges off, host_edges/budget pass, host-01 proceeds.
#[test]
fn stage4_full_chain_release_to_dispatched_host_passes_in_both_modes() {
    ...
    db.host_dispatch_state()
        .record_dispatch(&DispatchInsert {
            ...
            channel: "stable",
            wave: 0,
            target_closure_hash: "host-01-closure",
