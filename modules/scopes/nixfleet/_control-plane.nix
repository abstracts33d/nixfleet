  ...
  options.services.nixfleet-control-plane = {
    ...
    openFirewall = lib.mkOption {
      ...
      description = ''
        ...
        false because host-05 is on Tailscale; production deploys may
    ...
    freshnessWindowMinutes = lib.mkOption {
      ...
      description = ''
        ...
        the operator CI most-recently signed, so the effective freshness
    ...
    agentCnSuffix = lib.mkOption {
      ...
      description = ''
        FQDN suffix appended to agent cert CNs in the canonical form
        `agent-<machineId>.<suffix>`. Must match the issuance CA's
        dNSName constraint (D14 contract).

        Required - no default. The CP refuses to start if this option
        is unset. A silent fallback risks rejecting every agent
        checkin when the operator's CA was issued with a different
        suffix than the framework guessed. Set this explicitly to
        the suffix your issuance CA was minted with.
      '';
    };
    ...
    (lib.mkIf cfg.enable {
      ...
      systemd.services.nixfleet-control-plane = {
        ...
        serviceConfig = {
          ...
          # gitea-runner's TPM access in the operator CI flow.
