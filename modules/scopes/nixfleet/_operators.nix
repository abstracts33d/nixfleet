# Framework-level operators schema (platform-agnostic).
#
# Declares the `nixfleet.operators.*` option surface that nixfleet's
# own core modules (host-spec, _nixos, _darwin) reference for the
# primary user identity and root SSH keys. *Schema only* — no user
# creation, no SSH service wiring, no HM activation. Those live in
# the `operators` scope of nixfleet-scopes (or any equivalent fleet
# implementation).
#
# Auto-imported by mkHost so the option exists on every host even
# when no scope provides the implementation; the host then has a
# nixfleet.operators namespace to set, and downstream code can read
# `_primaryName` / `_adminSshKeys` to derive identity-dependent
# config (root SSH keys, hostSpec.userName fallback, agent secrets).
{
  config,
  lib,
  pkgs,
