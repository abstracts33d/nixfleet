{
  ...
  # Pre-computed OpenSSH pubkeys matching the agentKeypairs above.
  # Reading `${kp}/public.openssh` directly would be IFD and break
  # `nix flake check --no-build`. Values are deterministic from
  # `seedSalt + "/" + hostName` (see fixtures/agent-keypair); regen with
  # `nix-build -E 'import ./tests/harness/fixtures/agent-keypair { pkgs = import <nixpkgs> {}; hostName = "agent-01"; }'`
  # and read `result/public.openssh`. Scenarios mount the keypair's
  # private file into the agent AND declare `hosts.<name>.pubkey` here,
  # so any drift surfaces immediately as a signature mismatch.
  agentPubkeys = {
    agent-01 = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIF91bCCtzUJfrN+gNlIoWu+LQkW4U4DIn93vgbXuFJfJ";
    agent-02 = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOW/lyxaeyISG5LxDSbbLSa6P8utgVGMB1YIBiXxaECH";
    agent-99 = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFRRq8rXvyz2L0swRIf2RBCIKFZNAKgCXkeHSrOIyryS";
  };
