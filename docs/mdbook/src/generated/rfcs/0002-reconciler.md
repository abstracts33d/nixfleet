...
**Implementation status (2026-04-26 update — Phase 3 + early Phase 4 shipped).** The Phase 3 update landed the long-running CP server with mTLS agents posting `/v1/agent/checkin`. The hand-written `observed.json` is now an offline dev/test fallback; the live in-memory projection from agent check-ins replaces it as the operator-facing default (`crates/nixfleet-control-plane/src/observed_projection.rs`). The reconcile tick reads from this projection. **§4 step 0 (verify) and steps 1–6 (decision procedure) are implemented exactly as written — including the `OpenRollout` action emission.** Phase 4 added the persistence + activation wiring around the reconciler's plan output: SQLite-backed `pending_confirms` table, `/v1/agent/confirm` handler, magic-rollback timer (§3.2 `ConfirmWindow` deadline tracker), agent-side `nixos-rebuild switch` + local rollback. **What remains unimplemented from this RFC**: the dispatch loop that turns reconciler `OpenRollout` events into per-host `target` populations on `/v1/agent/checkin` responses; §4.1 edge ordering enforcement; §4.2 disruption budgets; §4.4 wave membership / soak / promote semantics; §4.5 the full `WaveSoaking → WavePromoted` transition; §4.6 `onHealthFailure` branches. Phase 4 follow-up work tracked in `docs/phase-4-entry-spec.md §3`.
