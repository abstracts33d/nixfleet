...
**Implementation status (2026-04-26 update — Phase 3 + early Phase 4 shipped on `phase-3-rolled-up` and `phase-4-rolled-up` branches).** Per-section coverage:

- §2 identity model: ✅ mTLS, ✅ 30d cert validity, ✅ 50% renewal, ✅ cert revocation set (enforced in `require_cn`). ❌ host-key-derived agent identity — agents generate fresh ed25519 keypairs at enrollment + each renewal instead of using the SSH host key. **Drift tracked in nixfleet#43** (Phase 6 candidate); weakens the "compromising the mTLS cert and compromising the host key are the same event" property — an attacker with filesystem access to `/run/agenix/agent-<host>-key` gets a usable identity without needing the SSH host key. Online fleet CA (used for /enroll and /agent/renew) is the related drift tracked in nixfleet#41.
- §4.1 `/agent/checkin`: ✅ implemented with richer body than the RFC minimum (pendingGeneration, lastEvaluatedTarget, lastFetchOutcome, uptimeSecs — all nullable, additive). `health` and `lastProbeResults` deferred to Phase 7 (probes generate the data).
- §4.2 `/agent/confirm`: ✅ wire shape + handler shipped. Covered by 4 integration tests (happy path / 410-when-no-pending / 403-on-CN-mismatch / 503-without-DB). Magic-rollback CP-side timer scans expired `pending_confirms` every 30s; agent-side local rollback wired. **Reachable end-to-end only after the dispatch loop lands** — without it, no pending_confirms rows ever exist.
- §4.3 `/agent/report`: ✅ shipped, but **body shape drifts from RFC**: this codebase uses `kind: ReportKind` enum + free-form `error` + `context: Value` instead of RFC's typed `event` + structured `details.{phase, exitCode, stderrTail}`. Decision pending: align wire to RFC OR amend RFC to match what shipped. Current shape covers fetch/verify failures naturally; RFC's shape is closer to activation-event use case. Decide when dispatch lands and `/report` is exercised with real activation data.
- §4.4 `/agent/closure/<hash>`: 🟡 narinfo-only forward shipped (returns 501 without `--closure-upstream`, 502 on upstream unreachable, 200 with body when reachable; 3 integration tests). **Full Nix-cache-protocol forwarding (nar streaming, signed-narinfo verification) is a follow-up.**
- §4.5 enrollment (`/v1/enroll`): ✅ shipped. Token signature verification against orgRootKey from trust.json is **enforced** (initial PR shipped a stub that accepted unsigned tokens; that gap was closed in the same Phase 3 chain). 3 integration tests cover happy path / signature tampering / nonce replay.
- §5 polling cadence: ✅ default 60s, ✅ exponential backoff with ±20% jitter on consecutive failures, capped at 8× base. ❌ per-host `nextCheckinSecs` shaping not implemented (cosmetic for small fleets).
- §6 versioning: ✅ `X-Nixfleet-Protocol: 1` header on every /v1/* request, CP middleware enforces. Missing-header currently accepted (forward-compat for agents deployed before the header landed); v2 will tighten to hard-reject.
- §7 security model: ✅ mTLS end-to-end. ❌ probe execution + signed evidence (Phase 7).
