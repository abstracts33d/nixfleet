...
The original design considered optional mTLS (allowing admin clients to connect without a client cert), but `axum-server` does not expose peer certificate information to the application layer. Making mTLS optional without application-level enforcement would leave agent endpoints unprotected. Required mTLS for all connections is the simpler, more secure approach.
