...
```sh
nix run .#test-vm                          # test with 'qemu' host (default)
nix run .#test-vm -- -h krach-qemu         # test with krach-qemu
nix run .#test-vm -- -h qemu --keep        # keep disk for inspection
```
...
|------|---------|-------------|
| `-h HOST` | qemu | Host config to install |
