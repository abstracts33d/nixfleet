...
- Agenix configuration lives in org darwinModules (injected via `fleet.nix`), not in `core/darwin.nix`
...
- Agenix is an org-level concern, injected via `mkOrg darwinModules`
