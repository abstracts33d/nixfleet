...
This config runs on:
- **NixOS** — full system control (krach, ohm, lab)
- **macOS** — nix-darwin with Home Manager (aether)
- **Any machine** — portable wrappers via `nix run`

Each platform has different capabilities. The config handles this with guards and platform-aware modules.
