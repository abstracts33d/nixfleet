...
Produit fonctionnel : 3 binaires Rust (agent, CP, CLI), 143 tests, démo 23min. Nix layer simplifié (Phase 0 terminée). Phase 1 (hardening) terminée : mTLS agent↔CP, API keys RBAC (readonly/deploy/admin), TLS-only, audit log + export CSV, refinery migrations. Gaps restants : single-tenant, pas de metrics, pas de dashboard.
