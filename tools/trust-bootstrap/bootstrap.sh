...
#
# Prerequisite: cp has converged onto a closure that declares the
...
agent_cn_suffix="fleet.example.com"
...
OPTIONS:
  --cp-host <host>            SSH target for the CP host (default: cp)
  --tpm-keyslot-name <name>    Keyslot name on cp (default: issuanceCA)
  --tpm-pubkey-path <path>     Override TPM pubkey path on cp
                               (default: /var/lib/nixfleet-tpm-keyslot/<name>/pubkey.raw)
  --agent-cn-suffix <fqdn>     Name constraint domain (default: fleet.example.com)
...
OUTPUT (in <output-dir>):
  ...
  fleet-issuance-ca.cert.pem   Issuance CA cert (ship to cp as
...
ssh -o BatchMode=yes -o ConnectTimeout=10 "$cp_host" true ||
  die "ssh $cp_host failed - check connectivity / authorized_keys"
...
ssh "$cp_host" test -f "$tpm_pubkey_path" ||
  die "TPM keyslot pubkey not found at $tpm_pubkey_path on $cp_host.
  Has cp converged onto a closure with
...
# ── 2. read TPM pubkey ───────────────────────────────────────────────
echo "▸ reading TPM pubkey from $cp_host:$tpm_pubkey_path"
...
ssh "$cp_host" cat "$tpm_pubkey_path" >"$tmp_pubkey_raw"
...
# D14 locked: name constraint = dNSName:fleet.example.com (and any
...
CONTENT:
  ...
  fleet-issuance-ca.cert.pem - issuance CA cert (ship to cp)
  trust-snippet.json         - trust.json fragment to merge into fleet config

NEXT STEPS (operator):

1. Ship the issuance CA cert to cp:
     scp $issuance_cert ${cp_host}:/etc/nixfleet/cp/issuance-ca.pem
...
4. Commit, push, cp converges. Verify:
     ssh ${cp_host} 'sudo journalctl -u nixfleet-control-plane -n 20'
